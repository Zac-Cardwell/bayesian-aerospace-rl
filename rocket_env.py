...
# Landing parameters
LANDING_PAD_Y = VIEWPORT_H / SCALE - 50 / SCALE
...
class RocketLandingEnv(gym.Env):
    ...
    def __init__(self, 
        ...
        Args:
            ...
            debug_enabled: Used for rendering fixtures, joints, AABBs, centers of mass 
            preview_only: Used for viewing static elements
        """
        super().__init__()
        ...
        obs_low = np.array([
            -2.0, 0.0, -10.0, -10.0, -np.pi, -5.0,  # pos, vel, angle, ang_vel
            0.0, 0.0,                               # leg contacts
            0.0, 0.8,                               # fuel norm, mass norm
            -20.0, -5.0,                            # wind x/y
            0.0                                     # time remaining
        ])
        obs_high = np.array([
            2.0, 2.0, 10.0, 10.0, np.pi, 5.0,
            1.0, 1.0,
            1.0, 1.2,
            20.0, 5.0,
            1.0
    ...
    def _create_terrain(self):
        ...
        # CHUNKS = 11 
        self.helipad_x1 = chunk_x[CHUNKS//2 - 1]  # index 4
        self.helipad_x2 = chunk_x[CHUNKS//2 + 1]  # index 6
        self.helipad_x = (self.helipad_x1 + self.helipad_x2) / 2
        self.helipad_y = LANDING_PAD_Y

        # Flatten from index 4 to 6 inclusive (3 points)
        height[CHUNKS//2 - 2 : CHUNKS//2 + 3] = LANDING_PAD_Y  # indices 3 to 7 
                
        smooth_y = [0.33 * (height[i - 1] + height[i] + height[i + 1]) for i in range(CHUNKS)]
        
        self.terrain = self.world.CreateStaticBody(
            fixtures=[
                fixtureDef(shape=edgeShape(vertices=[(chunk_x[i], smooth_y[i]), (chunk_x[i+1], smooth_y[i+1])]))
    ...
    def _create_rocket(self, spawn_on_pad=False):
        ...
        if spawn_on_pad:
            rocket_half_height = 17 / SCALE  # half of the rocket polygon height
            initial_x = self.helipad_x
            initial_y = self.helipad_y - rocket_half_height  # subtract instead of adding because render flips Y
            initial_y = self.helipad_y - rocket_half_height - 1
            angle = 0.0
        else:
        ...
        # Apply random initial velocity to set without impulse jitter
        # This gives small random velocity proportional to gravity - realistic drift
        if not spawn_on_pad:
    ...
    def reset(self, seed=None, options=None):
        ...
        self._create_rocket(spawn_on_pad=self.preview_only)
    ...
    def _get_observation(self):
        ...
        x_normalized = (pos[0] - VIEWPORT_W/SCALE/2) / (VIEWPORT_W/SCALE/2)
        y_normalized = (pos[1] - LANDING_PAD_Y) / (VIEWPORT_H/SCALE)
    ...
    def _compute_reward(self, action):
        ...
        # Distances
        dx = pos[0] - LANDING_PAD_X
        dy = pos[1] - LANDING_PAD_Y

        # Euclidean distance to landing pad
        dist = np.sqrt(dx * dx + dy * dy)

        # Speed magnitude
        speed = np.sqrt(vel.x * vel.x + vel.y * vel.y)

        shaping = (
            -100.0 * dist                  # Distance to pad
            -100.0 * speed                 # Velocity penalty
            -100.0 * abs(self.lander.angle)  # Orientation penalty
            + 10.0 * self.legs[0].ground_contact
            + 10.0 * self.legs[1].ground_contact
        )
        ...
        if self.game_over:
            ...
            if (
                ...
                and abs(vel.x) < 0.5
                and abs(vel.y) < 0.5
                and abs(self.lander.angle) < 0.3
            ):
                reward = 100.0
                reward += 50.0 * (self.fuel_remaining / FUEL_MASS)
            else:
                reward = -100.0
    ...
    def step(self, action):
        ...
        info = {
            ...
            'distance_to_pad': np.sqrt((self.lander.position[0] - LANDING_PAD_X)**2 + 
                                      (self.lander.position[1] - LANDING_PAD_Y)**2),
    ...
    def render(self):
        ...
        for obj in [self.terrain, self.lander] + self.legs:
            for fixture in obj.fixtures:
                ...
                if isinstance(shape, b2PolygonShape):
                    ...
                    vertices = [
                        (v[0] * SCALE, VIEWPORT_H - v[1] * SCALE) for v in vertices
                    ]
                ...
                # Edge
                    ...
                    v1 = transform * shape.vertex1
                    v2 = transform * shape.vertex2

                    p1 = (v1[0] * SCALE, VIEWPORT_H - v1[1] * SCALE)
                    p2 = (v2[0] * SCALE, VIEWPORT_H - v2[1] * SCALE)

                    pygame.draw.line(self.surf, color, p1, p2, 2)

                # Circle
                elif isinstance(shape, b2CircleShape):
                    pos = transform * shape.pos
                    center = (
                        int(pos[0] * SCALE),
                        int(VIEWPORT_H - pos[1] * SCALE),
                    )
        ...
        if self.enable_wind:
            wind_start = (50, 50)
            wind_end = (
                50 + int(self.wind_force[0] * 2),
                50 - int(self.wind_force[1] * 2),
            )
        ...
        fuel_ratio = max(0.0, min(1.0, self.fuel_remaining / FUEL_MASS))
